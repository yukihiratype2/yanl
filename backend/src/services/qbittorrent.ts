...
export class QbittorrentService {
  ...
  mapQbitPathToLocal(path: string): string {
    if (!path) return path;
  ...
  private encodeHashes(hashes: string | string[]): string {
  ...
  private async ensureLoggedIn(baseUrl: string): Promise<string> {
    if (this.sid !== null && this.sidBaseUrl === baseUrl) {
      return this.sid;
    }

    if (!this.loginPromise) {
      this.loginPromise = this.login(baseUrl).finally(() => {
  ...
  getQbitDownloadDir(mediaType: "anime" | "tv" | "movie"): string | undefined {
  ...
  async addTorrentByUrl(
    ...
    this.appendOptionalBoolean(formData, "skip_checking", options.skip_checking);
    this.appendOptionalBoolean(formData, "paused", options.paused);
    this.appendOptionalBoolean(formData, "root_folder", options.root_folder);
    if (options.rename) formData.append("rename", options.rename);
    this.appendOptionalNumber(formData, "upLimit", options.upLimit);
    this.appendOptionalNumber(formData, "dlLimit", options.dlLimit);
    this.appendOptionalNumber(formData, "ratioLimit", options.ratioLimit);
    this.appendOptionalNumber(formData, "seedingTimeLimit", options.seedingTimeLimit);
    this.appendOptionalBoolean(formData, "autoTMM", options.autoTMM);
    this.appendOptionalBoolean(formData, "sequentialDownload", options.sequentialDownload);
    this.appendOptionalBoolean(formData, "firstLastPiecePrio", options.firstLastPiecePrio);

    const response = await this.qbitFetch("/api/v2/torrents/add", {
      method: "POST",
  ...
  async getTorrents(options: GetTorrentsOptions = {}): Promise<QBitTorrentInfo[]> {
    const params = new URLSearchParams();
    ...
    const response = await this.qbitFetch(path);
    if (!response.ok) {
  ...
  async getManagedQbitTorrents(): Promise<QBitTorrentInfo[]> {
    const managedTags = this.getManagedQbitTags();
    ...
    const torrentsByTag = await Promise.all(
      Array.from(managedTags).map((tag) => this.getTorrents({ tag }))
    );
  ...
  private isSeedingStopped(torrent: Pick<QBitTorrentInfo, "state">): boolean {
    return torrent.state === "pausedUP" || torrent.state === "pausedDL";
  }

  async cleanupQbitTorrent(
    torrent: Pick<QBitTorrentInfo, "hash" | "tags" | "state">,
    managedTags: Set<string>,
    context: Record<string, unknown>
  ): Promise<void> {
    if (!this.hasManagedQbitTag(torrent, managedTags)) return;
    if (!this.isSeedingStopped(torrent)) return;
  ...
  async pauseTorrents(hashes: string | string[]): Promise<boolean> {
    const response = await this.qbitFetch("/api/v2/torrents/stop", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: this.encodeHashes(hashes),
    });
    return response.ok;
  }

  async resumeTorrents(hashes: string | string[]): Promise<boolean> {
    const response = await this.qbitFetch("/api/v2/torrents/start", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: this.encodeHashes(hashes),
    });
  ...
  async deleteTorrents(
    ...
    const response = await this.qbitFetch("/api/v2/torrents/delete", {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: params.toString(),
    });
  ...
  async getTorrentFiles(hash: string): Promise<QBitTorrentFile[]> {
    ...
    const response = await this.qbitFetch(`/api/v2/torrents/files?${params.toString()}`);
    if (!response.ok) {
  ...
  async testConnection(): Promise<{
    ...
    try {
      const baseUrl = this.getBaseUrl();
      await this.ensureLoggedIn(baseUrl);
      const response = await this.qbitFetch("/api/v2/app/version");
      if (response.ok) {
